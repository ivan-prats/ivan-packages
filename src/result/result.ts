 ...
 * //   ^^^^^^^^^^^^^^-- ErrorA | ErrorB
 */
export type ResultError<R extends Result<any, any>> = R extends Result<
  any,
  infer E
>
  ? E
 ...
 * //   ^^^^^^^^^^^^^^-- string
 */
export type ResultValue<R extends Result<any, any>> = R extends SuccessResult<
  infer V
>
  ? V
  : never;
 ...
 * //    ^^^^^-- never
 */
export function unfold<R extends Result<any, any>>(result: R): ResultValue<R> {
